...
#
# Writes: dist/Ace-Combat-3-Electrosphere-USA-Recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix Ace-Combat-3-Electrosphere-USA-Recomp \
  --exe-name Ace_Combat_3__Electrosphere__USA__Recompiled \
  --display-name "Ace Combat 3: Electrosphere (USA) Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Ace Combat 3: Electrosphere (USA) disc" \
